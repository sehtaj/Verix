...
const copy: Partial<Record<WorkflowScreen, { title: string; detail: string }>> = {
  ...
  investigating: {
    ...
    detail: "Tracing test output, generated coverage, and the selected source before drawing a conclusion.",
  },
  proposing_fix: {
...
function ActivityPhases({ screen }: { screen: WorkflowScreen }) {
  return (
    <ol className="mt-6 grid w-full max-w-3xl gap-px border border-outline-variant bg-outline-variant sm:grid-cols-2 lg:grid-cols-3">
      {(phases[screen] ?? []).map((phase, index) => (
        <li key={phase} className="flex items-center gap-3 bg-surface-low p-3 text-left font-heading text-xs">
          <span className={index === 0 ? "text-primary" : "text-muted-foreground"}>{index === 0 ? "▶" : "○"}</span>
          <span>{phase}</span>
        </li>
      ))}
    </ol>
...
export function WorkflowActivityBanner({ screen }: { screen: WorkflowScreen }) {
  ...
  return (
    <section className="mb-6 border border-primary bg-primary/5 p-4" aria-live="polite">
...
export function WorkflowLoadingPanel({ screen }: { screen: WorkflowScreen }) {
  ...
  return (
    <section className="mx-auto flex min-h-[60vh] max-w-4xl flex-col items-center justify-center border border-dashed border-outline-variant bg-surface-low/80 p-8 text-center" aria-live="polite">
